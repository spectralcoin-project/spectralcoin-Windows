...
#if defined(NDEBUG)
# error "Spectralcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1eaebaa75f31d174cbd1f7acd0a0da642179af7eb2182172dabecbede41c73d9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Spectralcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Spectralcoin Signed Message:\n";
...
{
    // Spectralcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Spectralcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Spectralcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Spectralcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Spectralcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Spectralcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1eaebaa75f31d174cbd1f7acd0a0da642179af7eb2182172dabecbede41c73d9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 13/November/2017 Trump Russia: US 'in peril over president's stance'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("878484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1510556839;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1554264;
        ...
        assert(block.hashMerkleRoot == uint256("0x01468d59d02f2084669e3950f7ff6e52a4357d15c992f8404c2d272993d505ae"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Spectralcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpectralcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpectralcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpectralcoinMiner(CWallet *pwallet)
{
    printf("SpectralcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spectralcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SpectralcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpectralcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpectralcoinMiner, pwallet));
}
